...
function run(){
  ...
  add('version',window.ADERENCIA_VERSION==='v1.0 RC49',window.ADERENCIA_VERSION||'ausente');
  ...
  add('ocr-not-eager',!document.querySelector('script[src*="tesseract.min.js"]'),window.Tesseract?'OCR já carregado':'OCR fora do startup');
  ...
  const ok=checks.every(x=>x.ok);window.ADERENCIA_RC49_HEALTH={ok,checks,cacheStats:window.ADERENCIA_RUNTIME_CACHE?.stats||null,ocr:window.ADERENCIA_OCR_LAZY||null,checkedAt:new Date().toISOString()};
  if(!ok)console.warn('RC49 integrity issues',checks.filter(x=>!x.ok));else console.info('RC49 integrity OK');
  return window.ADERENCIA_RC49_HEALTH;
}
...
window.ADERENCIA_RUN_RC49_CHECK=run;
})();