...
function mark(file){
 const last=window.ADERENCIA_SCAN_GRID_RC63?.last;
 ...
 const meta=Object.freeze({version:VERSION,source:'pdf-ocr-controlled',scannerVersion:last.version||window.ADERENCIA_SCAN_GRID_RC63?.version||'RC63',original:String(last.original||''),store:String(last.store||''),coverage,controlled,structuralCoverageFloor:controlled?.92:.95,distributionSafe:safe,identityRatio:Number(last.coverageProfile?.identityRatio),blankRows:Number(last.coverageProfile?.blankRows),blankCols:Number(last.coverageProfile?.blankCols),maxRowMissing:Number(last.coverageProfile?.maxRowMissing),maxColMissing:Number(last.coverageProfile?.maxColMissing),maxRun:Number(last.coverageProfile?.maxRun)});
...
window.ADERENCIA_SCAN_PROVENANCE_RC64={version:VERSION,mark,expectedName,safeProfile};
})();
