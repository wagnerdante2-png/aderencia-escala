...
function ensureButton(){
  ...
  b.title='Exporta o monitoramento mensal de todas as lojas para envio ao BI';
...
function buildRows(month,year){
  ...
  return Object.keys(stores).sort().map(store=>{
...
function exportMonitorExcel(){
  try{
    ...
    const rows=buildRows(month,year),sum=summary(rows,month,year);
    ...
    XLSX.writeFile(wb,`monitoramento_aderencia_${year}-${String(month).padStart(2,'0')}.xlsx`,{compression:true});
  }catch(e){
    console.error('Falha ao exportar monitoramento Excel',e);
    alert(`Não foi possível exportar o Excel: ${e.message||e}`);
  }
}
