...
function addRuleNote(){const rules=document.querySelector('.rules');if(!rules||rules.querySelector('[data-competence-integrity]'))return;const p=document.createElement('p');p.dataset.competenceIntegrity='true';p.innerHTML='<strong>Competência operacional:</strong> é sempre definida pelo início do período integral do espelho. Ex.: 11/06 a 10/07 = Junho; 11/07 a 10/08 = Julho. Se a escala cobrir somente parte desse ciclo (inclusive modelos 01–30/31), o cálculo usa a interseção disponível proporcionalmente, mas a competência continua sendo a do início do espelho.';rules.appendChild(p)}
function install(){
  canonicalizeDetails();repairHistoryFromEvidence();addRuleNote();
