...
function run(){
  ...
  add('version',window.ADERENCIA_VERSION==='v1.0 RC48',window.ADERENCIA_VERSION||'ausente');
  ...
  add('ocr-not-eager',!document.querySelector('script[src*="tesseract.min.js"]')||document.querySelector('script[src*="tesseract.min.js"]')?.dataset?.aderenciaOcr==='1',window.Tesseract?'carregado sob demanda':'não carregado no startup');
  ...
  const ok=checks.every(x=>x.ok);window.ADERENCIA_RC48_HEALTH={ok,checks,cacheStats:window.ADERENCIA_RUNTIME_CACHE?.stats||null,ocr:window.ADERENCIA_OCR_LAZY||null,checkedAt:new Date().toISOString()};
  if(!ok)console.warn('RC48 integrity issues',checks.filter(x=>!x.ok));else console.info('RC48 integrity OK');
  return window.ADERENCIA_RC48_HEALTH;
}
...
window.ADERENCIA_RUN_RC48_CHECK=run;
})();